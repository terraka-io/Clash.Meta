...
func (h *Http) DialContextTest(ctx context.Context, metadata *C.Metadata, opts ...dialer.Option) (tlsTime uint16, err error) {
	startTime := time.Now()
	var dialer C.Dialer = dialer.NewDialer(h.Base.DialOptions(opts...)...)
	...
	if err != nil {
		return 0, fmt.Errorf("%s connect error: %w", h.addr, err)
	}
